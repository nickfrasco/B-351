...
class DecisionTreeFactory:
    ...
    def _get_ranked_splits(self, splits):
        ...
        for i in range(n):
            for j in range(0,n-i-1):
                if self._calc_information_gain(splits[j],splits[i]) > self._calc_information_gain(splits[j+1],splits[i+1]):
                    splits[j],splits[i] = splits[j+1],splits[i+1]

            #info_gains.append(self._calc_information_gain(splits[0],splits[1]))
        #return sorted(info_gains,reverse = True)
    ...
    def _calc_information_gain(self, parent_node, children_nodes):
        ...
        before = parent_node.get_entropy(self.labels[parent_node.get_idxs()])

        children = []
        for c in children_nodes:
            children.append(c.get_entropy(self.labels[c.get_idxs()]))


        total = 0
        for x in children_nodes:
            total += len(x.get_idxs())

        after = 0
        for i in range(len(children_nodes)):
            after += (len(children_nodes[i].get_idxs()) / total) * children[i]
