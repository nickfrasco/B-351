    ...
    def isTerminal(self):
        ...
        for col in range(Board.WIDTH):
            for row in range(Board.HEIGHT):
                #vertical
                try:
                    if state[col][row] == state[col+1][row] == state[col+2][row] == state[col+3][row]:
                        return state[col][row] + 1
                except IndexError: pass

                #horizontal
                try:
                    if state[col][row] == state[col][row+1] == state[col][row+2] == state[col][row+3]:
                        return state[col][row] + 1
                except IndexError: pass

                # / diagonal
                try:
                    if state[col][row] == state[col+1][row+1] == state[col+2][row+2] == state[col+3][row+3]:
                        return state[col][row] + 1
                except IndexError: pass
